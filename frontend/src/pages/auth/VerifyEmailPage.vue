...
<template>
  <PublicLayout>
    <div class="rounded-xl border border-slate-200 bg-white p-6 text-center">
      <p v-if="state === 'loading'" class="text-sm text-slate-400">Verifying your email…</p>
      ...
      <template v-else>
        <p class="text-base font-medium text-slate-900">This verification link is invalid or has expired.</p>
        <p class="mt-1 text-sm text-slate-500">{{ message }}</p>

        <div v-if="auth.isAuthenticated && !auth.emailVerified" class="mt-4">
          <p v-if="resent" class="text-sm text-green-600">A fresh verification link is on its way.</p>
          <AppButton v-else :loading="resending" @click="onResend">Resend verification email</AppButton>
        </div>
